...
public class BestBranchToOrderService
{
    ...
    {
        var branches = await _branchRepo.GetRestaurantBranches(orderDto.BranchId);
    ...
    private Branch FindClosestBranch(double latitude, double longitude, IEnumerable<Branch> branches)
    {
        try
        {
            var sortedBranches = branches.OrderBy(b => _geoMaps.CalculateDistance(latitude, longitude, b.Lantitude, b.Longitude)).ToList();
